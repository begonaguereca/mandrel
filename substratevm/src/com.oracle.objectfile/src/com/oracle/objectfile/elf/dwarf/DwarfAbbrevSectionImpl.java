...
public class DwarfAbbrevSectionImpl extends DwarfSectionImpl {
    ...
    private int writeMethodLocationAbbrevs(@SuppressWarnings("unused") DebugContext context, byte[] buffer, int p) {
        ...
        pos = writeMethodLocationAbbrev(context, DwarfDebugInfo.DW_ABBREV_CODE_method_location_w_childer, buffer, pos);
        pos = writeMethodLocationAbbrev(context, DwarfDebugInfo.DW_ABBREV_CODE_method_location_inline, buffer, pos);
        pos = writeMethodLocationAbbrev(context, DwarfDebugInfo.DW_ABBREV_CODE_method_location_inline_w_chidren, buffer, pos);
    ...
    private int writeMethodLocationAbbrev(@SuppressWarnings("unused") DebugContext context, long abbrevCode, byte[] buffer, int p) {
        ...
        pos = writeAbbrevCode(abbrevCode, buffer, pos);
        pos = writeTag(DwarfDebugInfo.DW_TAG_subprogram, buffer, pos);
        if (abbrevCode == DwarfDebugInfo.DW_ABBREV_CODE_method_location_inline_w_chidren
                || abbrevCode == DwarfDebugInfo.DW_ABBREV_CODE_method_location_w_childer) {
            pos = writeFlag(DwarfDebugInfo.DW_CHILDREN_yes, buffer, pos);
        } else {
            pos = writeFlag(DwarfDebugInfo.DW_CHILDREN_no, buffer, pos);
        }
